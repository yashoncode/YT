...
class InFlightRequestCoalescerTest {
    @Test
    fun `concurrent requests for the same key share one operation`() =
        runBlocking {
    ...
    @Test
    fun `cancelling one waiter does not cancel the shared operation`() =
        runBlocking {
            val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
            try {
                val coalescer = InFlightRequestCoalescer<String, String>(scope)
    ...
    @Test
    fun `cancelling the last waiter cancels unobserved work`() =
        runBlocking {
            val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
            try {
                val coalescer = InFlightRequestCoalescer<String, String>(scope)
